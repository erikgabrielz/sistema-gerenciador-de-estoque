...
<section class="container">
    ...
    <article id="config-form">
        <form method="POST" id="form-order" action="<?php echo BASE_URL; ?>/servicos/addOrder" valid="true">
            
            <label class="label" for="name">Nome do cliente</label>
            <select class="input" name="name" id="name" autocomplete="off" required-input >
                <option>Selecione uma opção</option>
                <?php foreach($clients as $client): ?>
                    <option value="<?php echo $client['id'] ?>"><?php echo $client['name']. " - CPF nº " . $client['cpf'] ?></option>
                <?php endforeach; ?>
            </select>
            <div class="validate-message" id="name-message"></div>

            <label class="label" for="name">Marca/Modelo do celular</label>
            <select class="input" name="model" id="model" autocomplete="off" required-input >
                <option>Selecione uma opção</option>
                <?php foreach($models as $model): ?>
                    <option value="<?php echo $model['id'] ?>"><?php echo $model['product'] ?></option>
                <?php endforeach; ?>
            </select>
